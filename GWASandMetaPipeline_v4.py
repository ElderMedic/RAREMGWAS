...
# get correct major allele A2, calculate BETA from OR, drop all biallelic snps
    ...
    for run in loc_list:
        ...
        if cohort == 'Tacera': 
            ...
            print(cohort+": ",summary_origin.shape[0])
            ...
            summary_origin = pd.read_csv(run,sep='\s+')
#             print(summary_origin['SNP'].str.split(':'))
...
# make sure all snps across different cohorts use the same A1 effect allele (minor)
    ...
    for run in array_cohorts:
        ...
        summary_origin.to_csv(new_loc,sep='\t',index=0)
        print("work done on cohort ",run," count: ",summary_origin.shape[0])


def filterbySample(comparison,cwd=os.getcwd(),metaJob='METALresult_refEIRA_flipped_A1corr_maf1.TBL',out="METALresult_filtered.TBL"):
